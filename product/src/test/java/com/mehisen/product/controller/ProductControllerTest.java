...
class ProductControllerTest {
    ...
    void productById() throws Exception {
        ...
        product.setCurrency("EGP");

        when(productRepository.findById(anyString())).thenReturn(Optional.of(product));

        mockMvc.perform(get("/v1/product/some-random-id"))
                .andExpect(status().isOk())
                .andExpect(content().json("{\"id\":null,\"name\":\"Mi TV\",\"category\":null,\"price\":10000.0,\"currency\":\"EGP\",\"discount\":10.0,\"discountDescription\":null,\"imageURLs\":null}"));
    }
    ...
    void deleteProductById() throws Exception {
        ...
        mockMvc.perform(delete("/v1/deleteProduct/some-random-id"))
                .andExpect(status().isOk())
